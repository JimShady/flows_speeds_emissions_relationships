...
model_results <- data.frame(road_type = NA,
                            pollutant = NA,
                            rsquared = NA, stringsAsFactors = F)

for (i in 1:length(sources)) {
  
  for (j in 1:length(road_types)) {
  
  model <- lm(ten_km_contribution ~ length + speed + total_light + total_heavy,
             data=training_data[training_data$pollutant == sources[i] & training_data$desc_term == road_types[j],])

  models[[counter]] <- model
  
  model_results[counter,] <- c(road_types[j], sources[i],  round(summary(models[[counter]])$adj.r.squared,3) )
...
Now use the regression equation on the testing 20% data FIX THIS BIT
...
for (i in 1:length(sources)) {
  ...
  testing_data[testing_data$pollutant == sources[i] & testing_data$desc_term == road_types[j],'prediction'] <-   predict(models[[i]], testing_data[testing_data$pollutant == sources[i] & testing_data$desc_term == road_types[j],])
                                  }
                            }
